  ...
  //  $counts=0;
    ...
    if (isset($resDataGrid[0]['id'])) {
        foreach ($resDataGrid as $flow) {
            $flows[] = array(
                "Id" => intval($flow["id"]),
                "KurumGrupID" => intval($flow["kurumGrupID"]), 
...
* @since 31-01-2018
 */
$app->get("/pkUpdateMakeActiveOrPassive_sysKurumTurleri/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysKurumTurleriBLL');
    $headerParams = $app->request()->headers();
    if (!isset($headerParams['X-Public'])) {
        throw new Exception('rest api "pkUpdateMakeActiveOrPassive_sysKurumTurleri" end point, X-Public variable not found');
    }
    $Pk = $headerParams['X-Public'];      
    $vId = NULL;
    if (isset($_GET['Id'])) {
...
* @since 31-01-2018
 */
$app->get("/pkInsert_sysKurumTurleri/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysKurumTurleriBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkInsert_sysKurumTurleri" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
    ...
    $stripper->strip();
    if($stripper->offsetExists('KurumGrupID')) $vkurumGrupID = $stripper->offsetGet('KurumGrupID')->getFilterValue();
...
* @since 31-01-2018
 */
$app->get("/pkUpdate_sysKurumTurleri/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory(); 
    $BLL = $app->getBLLManager()->get('sysKurumTurleriBLL');  
    $headerParams = $app->request()->headers();
    if(!isset($headerParams['X-Public'])) throw new Exception ('rest api "pkUpdate_sysKurumTurleri" end point, X-Public variable not found');    
    $pk = $headerParams['X-Public'];
...
$app->get("/pkDelete_sysKurumTurleri/", function () use ($app ) {
    $stripper = $app->getServiceManager()->get('filterChainerCustom');
    $stripChainerFactory = new \Services\Filter\Helper\FilterChainerFactory();    
    $BLL = $app->getBLLManager()->get('sysKurumTurleriBLL');   
    $headerParams = $app->request()->headers();
